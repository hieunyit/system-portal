...
// @Accept json
// @Produce json
// @Param request body openvpndto.BulkDisconnectUsersRequest true "Bulk disconnect users request"
// @Success 200 {object} response.SuccessResponse{data=openvpndto.DisconnectResponse} "Users disconnected successfully"
...
func (h *DisconnectHandler) BulkDisconnectUsers(c *gin.Context) {
	var req openvpndto.BulkDisconnectUsersRequest
...
// @Param request body openvpndto.DisconnectUserRequest true "Disconnect user request"
// @Success 200 {object} response.SuccessResponse{data=openvpndto.DisconnectResponse} "User disconnected successfully"
...
func (h *DisconnectHandler) DisconnectUser(c *gin.Context) {
	...
	var req openvpndto.DisconnectUserRequest
	if err := c.ShouldBindJSON(&req); err != nil {
...
func (h *DisconnectHandler) convertBulkDisconnectResult(result *usecases.BulkDisconnectResult) openvpndto.DisconnectResponse {
	return openvpndto.DisconnectResponse{
...
func (h *DisconnectHandler) convertSingleDisconnectResult(result *usecases.DisconnectResult) openvpndto.DisconnectResponse {
	response := openvpndto.DisconnectResponse{
		Success:           result.Success,
...
func (h *DisconnectHandler) convertUsecaseValidationErrors(usecaseErrors []usecases.UserValidationError) []openvpndto.UserValidationError {
	if len(usecaseErrors) == 0 {
		return nil
	}

	dtoErrors := make([]openvpndto.UserValidationError, len(usecaseErrors))
	for i, err := range usecaseErrors {
		dtoErrors[i] = openvpndto.UserValidationError{
...
func (h *DisconnectHandler) convertConnectionInfo(connInfo *entities.ConnectedUser) *openvpndto.UserConnectionInfo {
	if connInfo == nil {
		return nil
	}

	return &openvpndto.UserConnectionInfo{
